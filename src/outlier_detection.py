...
class OutlierDetector:
    ...
    def handle_outliers(self, df: pd.DataFrame, method="remove", **kwargs) -> pd.DataFrame:
        ...
        if method == "remove":
            ...
            df_cleaned = df[~outliers[numeric_cols].any(axis=1)]
            logging.info("Removing outliers from the dataset.") 
        elif method == "cap":
    ...
    def visualize_outliers(self, df: pd.DataFrame, features: list) -> None:
        """
        Visualizes the outliers in the specified features using boxplots.

        Parameters:
        df (pd.DataFrame): The dataframe containing the features to be visualized.
        features (List[str]): The list of feature names to visualize outliers for.
        """
        logging.info(f"Visualizing outliers for features: {features}")
        for feature in features:
            plt.figure(figsize=(14,6))
            sns.boxplot(x=df[feature],palette="rocket_r")
...
if __name__ == "__main__":
    ...
    # outlier_detector.visualize_outliers(df_cleaned, features=["Annual_premium"])
    pass